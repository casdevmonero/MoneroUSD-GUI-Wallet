...
const LOAN_PARAMS = {
  BTC: {
    max_ltv: 0.65,            // 65% max loan-to-value
    liquidation_ltv: 0.80,    // 80% triggers liquidation
  ...
  XMR: {
    max_ltv: 0.55,            // 55% max loan-to-value
    liquidation_ltv: 0.75,    // 75% triggers liquidation
...
const BLOCKS_PER_DAY = 1440; // 60s block time -> 1440 blocks/day
...
// Feed interest revenue to the swap-service yield pool

async function addToYieldPool(atomicAmount) {
  if (atomicAmount <= 0n) return;
  try {
    // The swap-service tracks the yield pool — we call its internal endpoint
    // For now, we track locally and the staking-service reads from swap-service
    // Interest revenue distribution happens at accrual time
    console.log(`[interest] ${Number(atomicAmount) / 1e8} USDm allocated to yield pool`);
  } catch (_) {}
}
