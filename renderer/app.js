...
(function () {
  ...
  function updateSendBalanceHint(rawInput) {
    ...
    if (raw) {
      ...
      if (amt > 0n) {
        const fee = estimateTxFee(amt);
        const totalNeeded = amt + fee;
        if (amt <= fee || amt > available) {
          // Amount exceeds balance entirely or is less than the fee
          message += ' • Insufficient balance';
          isError = true;
        } else if (totalNeeded > available) {
          // Fee will be auto-deducted from send amount
          const actualSend = amt - fee;
          message += ' • Fee ~$' + formatAmount(fee) + ' (will send $' + formatAmount(actualSend) + ')';
        } else {
          const remaining = available - totalNeeded;
          message += ' • Remaining $' + formatAmount(remaining) + ' (fee ~$' + formatAmount(fee) + ')';
        }
      }
    }
  ...
  const LOCAL_NODE_HINT = ' Check your connection or try refreshing. Local nodes: run USDmd (port 17750) and ./start-wallet-rpc.sh (port 27750).';
  ...
  function bindSwap() {
    ...
    function swapStatusLabel(status, swap) {
      if (status === 'awaiting_deposit' && swap) {
        const conf = Number(swap.deposit_confirmations || 0);
        const reqd = Number(swap.deposit_confirmations_required || 0);
        if (swap.deposit_tx && conf === 0) return 'Deposit in mempool';
        if (swap.deposit_tx && conf > 0 && reqd > 0) return `Pending (${conf}/${reqd})`;
      ...
      swaps.forEach((s) => {
        ...
        if (isSwapPending(s.status)) {
          ...
          item.insertAdjacentHTML('beforeend',
            '<span class="swap-history-status ' + escHtml(swapStatusClass(s.status)) + '">' + escHtml(swapStatusLabel(s.status, s)) + '</span>');
    ...
    function viewSwap(record) {
      ...
      if (record.status === 'minted') {
        ...
        setStatus(escHtml(swapStatusLabel(record.status, record)));
      }
    }
    ...
    async function submitBurn() {
      ...
      // Use cached balance for instant check, fall back to immediate RPC
      let available = lastUsdmBalanceAtomic || 0n;
      if (available === 0n) {
        const balance = await rpcImmediate('get_balance', { account_index: 0, all_accounts: false, all_assets: false, asset_type: ASSET_USDM, strict: false }, { timeoutMs: 15000 });
      ...
      const res = await rpcImmediate('transfer', {
    ...
    async function pollSwapStatus() {
      ...
      try {
        ...
        persistSwap({
          swap_id: swapId, status, minted_tx: res.minted_tx, payout_tx: res.payout_tx, error: res.error,
          deposit_tx: res.deposit_tx, deposit_confirmations: res.deposit_confirmations,
          deposit_confirmations_required: res.deposit_confirmations_required,
        });
        if (status === 'awaiting_deposit') {
          const conf = Number(res.deposit_confirmations || 0);
          const reqd = Number(res.deposit_confirmations_required || 0);
          if (res.deposit_tx && conf === 0) {
            setStatus('Deposit detected in mempool. Waiting for confirmations\u2026');
          } else if (res.deposit_tx && conf > 0 && reqd > 0) {
            setStatus(`Deposit pending \u2014 ${conf}/${reqd} confirmations\u2026`);
          } else {
            setStatus('Waiting for deposit\u2026');
          }
    ...
    async function pollAllPendingSwaps() {
      ...
      for (const s of pending) {
        try {
          const res = await swapFetch(`/api/swaps/${s.swap_id}`);
          if (!res) continue;
          const confChanged = res.deposit_confirmations != null && res.deposit_confirmations !== s.deposit_confirmations;
          if ((res.status && res.status !== s.status) || confChanged) {
            statusChanged = true;
            persistSwap({
              swap_id: s.swap_id, status: res.status,
              minted_tx: res.minted_tx, payout_tx: res.payout_tx, error: res.error,
  ...
  function bindSend() {
    ...
    btnSend?.addEventListener('click', async () => {
      ...
      let amountAtomic = parseDecimalToAtomic(amountStr, USDM_DECIMALS);
      if (!(amountAtomic > 0n)) {
        showMessage('sendMessage', 'Invalid amount.', true);
        return;
      }
      const estimatedFee = estimateTxFee(amountAtomic);
      let sendAtomic = amountAtomic;
      let feeDeducted = false;
      // Use cached balance for instant pre-check (no RPC wait)
      let available = lastUsdmBalanceAtomic || 0n;
      // If cached balance is zero, do a quick immediate RPC fetch
      if (available === 0n) {
        try {
      ...
      if (available > 0n && available < amountAtomic + estimatedFee) {
        sendAtomic = amountAtomic - estimatedFee;
        feeDeducted = true;
        if (sendAtomic <= 0n) {
          updateSendBalanceHint(amountStr);
          showMessage('sendMessage', 'Balance too low to cover the tx fee (~$' + formatAmount(estimatedFee) + ').', true);
          return;
        }
      }
      ...
      const confirmMsg = 'Confirm Transaction\n\n'
        ...
        + 'Amount: $' + formatAmount(sendAtomic) + ' USDm\n'
        + 'Est. fee: ~$' + formatAmount(estimatedFee) + '\n'
        + (feeDeducted ? '(Fee deducted from send amount)\n' : '')
      ...
      try {
        ...
        if (sendMsgEl) {
          let html = '<div style="font-size:1.1em;font-weight:600;margin-bottom:6px">\u2705 Transaction Complete</div>';
          html += '<div style="margin-bottom:4px">Amount: <strong>$' + formatAmount(sendAtomic) + ' USDm</strong></div>';
          if (txFee) html += '<div style="margin-bottom:4px">Fee: $' + formatAmount(BigInt(txFee)) + '</div>';
          if (txHash) {
            html += '<div>TX: <a href="' + explorerBase + '/tx/' + escHtml(txHash) + '" target="_blank" style="color:#4fc3f7;text-decoration:underline">'
              + escHtml(txHash.slice(0, 16)) + '\u2026' + escHtml(txHash.slice(-8)) + '</a></div>';
          }
          sendMsgEl.innerHTML = html;
        ...
        // Refresh balance in background — don't block the UI
        refreshBalances({ force: true }).catch(() => {});
        refreshTransfers().catch(() => {});
      } catch (e) {
    ...
    btnCreate.addEventListener('click', async () => {
      ...
      if (isElectron) {
        ...
        if (!result.ok) {
          ...
          markStep(errorStep, 'error', 'Setup needs attention');
          // Format error with styled terminal-style commands
          const errMsg = (result.error || 'Setup failed')
            .replace(/\n\n/g, '<br><br>')
            .replace(/\n/g, '<br>')
            .replace(/(\/bin\/bash -c[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(brew install[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(git clone[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(sudo port[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(cp build[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(xcode-select[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>')
            .replace(/(sudo xcodebuild[^<]+)/g, '<code style="display:block;background:#111;padding:8px 12px;border-radius:6px;margin:4px 0;font-family:monospace;font-size:0.85em;color:#0f0;user-select:all;cursor:text">$1</code>');
          // Add copy buttons to each code block (CSP-safe: no inline handlers)
          const withCopy = errMsg.replace(/<code ([^>]+)>([^<]+)<\/code>/g, (match, style, cmd) => {
            return '<div style="position:relative;margin:4px 0"><code ' + style + '>' + cmd + '</code>'
              + '<button class="node-copy-btn" data-cmd="' + cmd.replace(/"/g, '&quot;') + '" '
              + 'style="position:absolute;top:4px;right:4px;background:#333;color:#fff;border:1px solid #555;border-radius:4px;padding:2px 8px;font-size:0.75em;cursor:pointer">Copy</button></div>';
          });
          const msgEl = document.getElementById('nodeMessage');
          if (msgEl) {
            msgEl.innerHTML = withCopy
              + '<br><button class="btn btn-primary node-retry-btn" style="margin-top:12px;width:100%">'
              + '\u25B6 Create Node Again</button>';
            msgEl.classList.add('error');
            // Attach event listeners (CSP-safe)
            msgEl.querySelectorAll('.node-copy-btn').forEach(btn => {
              btn.addEventListener('click', () => {
            ...
            const retryBtn = msgEl.querySelector('.node-retry-btn');
            if (retryBtn) {
              retryBtn.addEventListener('click', () => {
                // Clear error message and reset step UI before retrying
                msgEl.innerHTML = '';
                msgEl.classList.remove('error');
                showMessage('nodeMessage', 'Retrying setup — re-checking dependencies\u2026', false);
                btnCreate.disabled = false;
                btnCreate.click();
              });
            }
          }
          else showMessage('nodeMessage', result.error || 'Setup failed', true);
          btnCreate.disabled = false;
  ...
  function bindRefresh() {
    document.getElementById('btnRefresh')?.addEventListener('click', async () => {
      ...
      try {
        await refreshAddress();
        const { parsed } = await fetchUsdmBalance({ immediate: true });
        const display = parsed.unlocked_balance > 0n ? parsed.unlocked_balance : parsed.balance;
  ...
  function bindImport() {
    ...
    document.getElementById('btnImport')?.addEventListener('click', async () => {
      ...
      if (wordCount !== 12 && wordCount !== 24 && wordCount !== 25) {
        showMessage('importMessage', 'Seed must be 12, 24, or 25 words (USDm/Monero use 25). You have ' + wordCount + ' words.', true);
        return;
      }
      ...
      try {
        ...
        if (/Electrum-style word list failed verification/i.test(msg)) {
          friendly = 'Seed verification failed. Check: all words spelled correctly, same language, no extra characters. USDm/Monero use 25 words; the last word is a checksum.';
  ...
  function showMainApp() {
    ...
    // Reset the inactivity lock state so the auto-timeout timer restarts for this session
    unlockWallet();
  }
  ...
  function bindWelcome() {
    ...
    // Logout button — must work even when wallet-rpc is blocked by a long sync.
    // Fire close_wallet in background (don't await) so UI resets instantly.
    ...
    window.electronAPI.onUpdateDownloaded((data) => {
      ...
      btnAction.onclick = () => {
        ...
        setTimeout(() => {
          ...
          bannerText.textContent = `Update v${data.version} ready. Quit the app and reopen to install.`;
          btnAction.onclick = () => window.electronAPI.updateInstall();
        }, 5000);
      };
      // Show "Later" dismiss button so users can quit/postpone without restarting now
      btnDismiss.textContent = 'Later';
      btnDismiss.classList.remove('hidden');
      btnDismiss.onclick = () => { banner.classList.add('hidden'); };
      bannerText.textContent = `Update v${data.version} ready. Restart to install.`;
      banner.classList.remove('hidden');
  ...
  function bootstrap() {
    ...
    // Verify wallet files actually exist on disk — localStorage may persist
    // across updates/reinstalls even when wallet files have been removed.
    if (isOnboarded() && window.electronAPI && typeof window.electronAPI.checkWalletExists === 'function') {
      window.electronAPI.checkWalletExists().then((exists) => {
        if (exists) {
          showMainApp();
          initMainApp();
        } else {
          // Stale onboarding state — clear it and show welcome page
          storageSet(WALLET_ONBOARDED_KEY, '');
          try { localStorage.removeItem(walletListStorageKey()); } catch (_) {}
          showWelcomePage();
        }
      }).catch(() => {
        // If check fails, fall back to localStorage-only check
        showMainApp();
        initMainApp();
      });
    } else if (isOnboarded()) {
