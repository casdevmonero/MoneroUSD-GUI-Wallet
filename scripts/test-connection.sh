...
#   2. USDM_DAEMON_ADDRESS=http://localhost:17750 ./start-wallet-rpc.sh
#   3. (Browser) node server.js
# Usage: ./scripts/test-connection.sh [base_url]
#   base_url default: http://localhost:27750 (wallet RPC direct). Use http://localhost:3000 to test via app server.

set -e
BASE="${1:-http://localhost:27750}"
...
rpc() {
  ...
  if [[ "$BASE" == *":3000"* ]]; then
    curl -s -X POST "$BASE/json_rpc" \
      ...
      -H "x-wallet-rpc-url: http://localhost:27750" \
...
echo "2. set_daemon (http://localhost:17750)"
out=$(rpc "set_daemon" '{"address":"http://localhost:17750","trusted":true}')
if echo "$out" | grep -q '"result"'; then
  echo "   OK"
else
  echo "   FAIL: $out"
...
EXTRA_HEADER=""
[[ "$BASE" == *":3000"* ]] && EXTRA_HEADER="-H x-wallet-rpc-url: http://localhost:27750"
...
echo ""
echo "All steps passed. App should work: open Settings, set Daemon URL to http://localhost:17750, Save, then Refresh on dashboard."
